  ...
  plugins: [
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use((req, res, next) => {
          ...
          // URLs with extensions: pass through if the file exists on disk OR if it's a
          // dynamic work/insights page (generated by Express, never on disk).
          // Unknown dotted URLs would otherwise fall through to Vite's SPA fallback.
          if (urlPath.includes('.')) {
            if (
              /^\/work\/[a-z0-9][a-z0-9-]*\.html$/i.test(urlPath) ||
              /^\/insights\/[a-z0-9][a-z0-9-]*\.html$/i.test(urlPath)
            ) return next()  // let Express dynamic handler decide (serves page or 404)
